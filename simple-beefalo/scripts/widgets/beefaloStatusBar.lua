...
local BeefaloStatusBar = Class(Widget, function(self, owner, config)
    ...
    self.root:SetPosition(self.CONFIG.ROOT_X + self.CONFIG.ROOT_X_HIDDEN, self.CONFIG.ROOT_Y)
...
function BeefaloStatusBar:SlideOut()
    ...
    self:StopTimer()
    self.root:CancelMoveTo()
    self.root:MoveTo(self.root:GetPosition(), {x = self.CONFIG.ROOT_X + self.CONFIG.ROOT_X_HIDDEN, y = self.CONFIG.ROOT_Y, z = 0}, 0.5, function() self:Hide() end)
...
function BeefaloStatusBar:Reposition()
    if self:IsVisible() then
        self:MoveTo(self:GetPosition(), {x = self.CONFIG.BASE_X, y = self.CONFIG.BASE_Y, z = 0}, 0.15)
    else
        self:SetPosition(self.CONFIG.BASE_X, self.CONFIG.BASE_Y)
    end
end
